...
class api extends restful_api {
	...
	function xoa_users(){
		...
		header('location:http://laravel.com.vn/docs/');
	}
		...
		if($this->method == 'POST'){
			...
			if($con->connect_error){
				...
				if(is_uploaded_file($_FILES["logo"]["tmp_name"]) ){
					$tmp_file = $_FILES["logo"]["tmp_name"];
					$upload_dir = "images/".$img_name;
					$sql  = "UPDATE teams set name='$name',description='$mota',logo='$img_name',leader_id='$leader_id' where id='$id'";
					if(move_uploaded_file($tmp_file, $upload_dir) && $con->query($sql)){
						$res["MESSAGE"]	="update thanh cong ";
						$res["STATUS"]	=200;
					}else{
